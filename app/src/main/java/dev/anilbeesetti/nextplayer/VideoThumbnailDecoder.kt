...
class VideoThumbnailDecoder(
    ...
    override suspend fun decode(): DecodeResult {
        ...
        val rawBitmap = MediaMetadataRetriever().use { nativeRetriever ->
            buildMediaInfo(source).use { mediaInfo ->
                nativeRetriever.setDataSource(source)

                // First, try to get embedded picture (album art/metadata thumbnail).
                val embeddedPictureBitmap = nativeRetriever.embeddedPicture?.let { pictureBytes ->
                ...
                return@use when (strategy) {
                    is ThumbnailStrategy.FirstFrame -> {
                        nativeRetriever.getFrameAtTime(0) ?: mediaInfo?.getFrameAt(0)
                    }

                    is ThumbnailStrategy.FrameAtPercentage -> {
                        val timeMs = (videoDuration * strategy.percentage).toLong()
                        val timeUs = timeMs * 1000
                        nativeRetriever.getFrameAtTime(timeUs) ?: mediaInfo?.getFrameAt(timeMs)
                    }

                    is ThumbnailStrategy.Hybrid -> {
                        val firstFrame = nativeRetriever.getFrameAtTime(0) ?: mediaInfo?.getFrameAt(0)
    ...
    private fun buildMediaInfo(source: ImageSource): MediaInfo? {
        val metadata = source.metadata
        return runCatching {
            when {
                metadata is ContentMetadata -> {
                    MediaInfoBuilder()
                        .from(options.context, metadata.uri.toAndroidUri())
                        .build()
                }

                source.fileSystem === FileSystem.SYSTEM -> {
                    MediaInfoBuilder()
                        .from(source.file().toFile().path)
                        .build()
                }
    ...
    private inline fun <T> MediaInfo?.use(block: (MediaInfo?) -> T): T {
        try {
            return block(this)
        } finally {
            this?.release()
